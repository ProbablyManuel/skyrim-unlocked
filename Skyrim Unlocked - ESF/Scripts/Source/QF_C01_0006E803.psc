...
; this is copied here because werewolf mods may edit C01QuestScript
Utility.Wait(2.0)

Actor selectedObserver = (kmyQuest.CentralQuest as CompanionsHousekeepingScript).GetFavoriteQuestgiver()
if selectedObserver != kmyQuest.Aela
	selectedObserver = kmyQuest.Farkas
	(kmyQuest.CentralQuest as CompanionsHousekeepingScript).FarkasIsObserver = true
else
	(kmyQuest.CentralQuest as CompanionsHousekeepingScript).AelaIsObserver = true
endif

kmyQuest.Observer.ForceRefTo(selectedObserver)
(kmyQuest.CentralQuest as CompanionsHousekeepingScript).TrialObserver.ForceRefTo(selectedObserver)
kmyQuest.DebugFollower.ForceRefTo(selectedObserver)
;END CODE
EndFunction
;END FRAGMENT
...
Alias_Dungeon.GetLocation().SetCleared(false)
