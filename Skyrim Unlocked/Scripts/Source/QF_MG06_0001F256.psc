...
Game.GetPlayer().AddPerk(MGEnthirVendorPerk) ; Added by USKP 1.3.3 (Bug #6693)
...
;BEGIN CODE
SetObjectiveCompleted(70) ; USKP 2.0.5
DialogueWinterholdCollegeQuestScript DQScript = MGDialogueQuest as DialogueWinterholdCollegeQuestScript
DQScript.GateOpen=1
Game.GetPlayer().Moveto(MG05PlayerStartMarker)
;USKP 2.0.5 - Don't move followers if they've been told to wait somewhere else.
if (Follower.GetReference() != None)
 if(Follower.GetActorReference().GetAV("WaitingForPlayer") == 0)
  Follower.GetReference().MoveTo(MG05FollowerMarker)
 EndIf
endif
if (AnimalFollower.GetReference() != None)
 if(AnimalFollower.GetActorReference().GetAV("WaitingForPlayer") == 0)
...
ObjectReference Property dunMzulftEnableMarker01  Auto  
