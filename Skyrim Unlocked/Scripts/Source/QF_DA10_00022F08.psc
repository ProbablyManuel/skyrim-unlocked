...
Alias_Tyranus.TryToEvaluatePackage()

;Tyranus goes inside
pHouseEntryScene.Start()

;Tyranus is no longer essential
Alias_Tyranus.GetActorReference().GetActorBase().SetEssential(false)

;unlock the doors
Alias_MainDoor.GetReference().Lock(False)
Alias_BasementDoor.GetReference().BlockActivation()
Alias_Altar.GetReference().SetNoFavorAllowed()
Alias_MaceofMolagBal.GetReference().SetNoFavorAllowed()
Alias_RustyMaceTriggerREF.GetReference().SetNoFavorAllowed()
;END CODE
EndFunction
;END FRAGMENT
...
Alias_MainDoor.GetRef().BlockActivation()

;remove Tyranus from his crime faction
Alias_Tyranus.GetActorReference().SetCrimeFaction(None)
Alias_Tyranus.TryToRemoveFromFaction(CrimeFactionReach)
;END CODE
EndFunction
;END FRAGMENT
...
;unlock basement
Alias_BasementDoor.GetReference().BlockActivation(False)
;END CODE
EndFunction
;END FRAGMENT
...
Alias_Tyranus.TryToAddtoFaction(pPlayerEnemyFaction)
;USKP 2.0.2 - Dead folk can't start combat.
if( !Alias_Tyranus.GetActorReference().IsDead() )
	Alias_Tyranus.GetActorReference().StartCombat(Game.GetPlayer())
EndIf
;END CODE
EndFunction
;END FRAGMENT
...
Alias_Altar.GetReference().PlayAnimation("Down")
Alias_RustyMaceIntREF.TryToDisable()
DA10HauntingController.SetStage(90)

;Unlock the doors
Alias_MainDoor.GetReference().BlockActivation(False)

;Disable static bed/ enable non static bed
DA10NonStaticBedEnabler.enable()

;Move and enable Logrolf
Alias_Logrolf.TryToMoveto(Alias_CaptiveMarker.GetRef())
Alias_Logrolf.TryToEnable()
Alias_Logrolf.GetActorReference().PlaceatMe(BoundCaptiveMarker)
Alias_Logrolf.GetActorReference().SetRestrained(1)

Alias_MapMarker.GetReference().AddtoMap()

Utility.Wait(2)
Alias_Altar.GetReference().BlockActivation()
...
;BEGIN CODE
Alias_Yngvar.TryToEvaluatePackage()
;END CODE
EndFunction
;END FRAGMENT
...
Actor pLogrolf = Alias_Logrolf.GetActorReference()
DA10LogrolfMolagBalScene02.Start()
;Make Logrolf Essential
Alias_Logrolf.GetActorReference().GetActorBase().SetEssential(True)
...
utility.wait(0.5)
Alias_Logrolf.GetActorReference().Resurrect()
...
Alias_MainDoor.GetReference().Lock()
Alias_BasementDoor.GetReference().BlockActivation(False)
...
Alias_Altar.GetReference().PlayAnimation("Up")
...
Alias_RustyMaceStatic.TryToDisable()
Alias_MaceofMolagBal.TryToDisable()
Game.GetPlayer().AddItem(Alias_PlayerRustyMace.GetReference())
...
Alias_Logrolf.GetActorReference().GetActorBase().SetEssential(False)
...
;Alias_AltarofMolagBal.GetReference().Activate(Game.GetPlayer())
...
Alias_MolagBalFinalTalking.GetReference().Activate(Game.GetPlayer())
;END CODE
EndFunction
;END FRAGMENT
...
;BEGIN CODE
Alias_Tyranus.TryToEnable()
Alias_Tyranus.TryToMoveto(DA10TyranusMarkerREF)
Alias_Yngvar.TryToEvaluatePackage()
Alias_Yngvar.TryToMoveto(DA10YngvarMarkerREF)
...
;Release Logrolf
Alias_Altar.GetRef().PlayAnimation("Down")
Alias_Logrolf.GetActorReference().EvaluatePackage()

;Delete Rusty Mace
Alias_PlayerRustyMace.GetReference().Delete()
