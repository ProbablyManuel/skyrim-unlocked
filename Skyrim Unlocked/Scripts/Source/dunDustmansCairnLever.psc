...
Auto STATE pulledPosition
	...
	if (C01 == NONE)	;Added by Skyrim Unlocked.
		C01 = Game.GetFormFromFile(0x06E803, "Skyrim.esm") as Quest	;C01 isn't a property on the second lever so we need to set it here.
	endif
	
	if (C01.IsRunning()) ;Added by Skyrim Unlocked.
...
; 		debug.Trace(self + ": my Trigger " + C01CompanionTriggerBox + " has this many companions " + C01CompanionTriggerBox.getTriggerObjectCount())
	...
	; 			debug.Trace(self + ": will not work because a companion is in the box")
		...
		cannotBeActivatedMessage.show()	;Added by Skyrim Unlocked.
	endif
	endEVENT
endState
